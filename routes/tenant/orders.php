...
Route::post('order', SubmitOrderController::class)->name('order.store')->middleware('throttle:form-write');
...
Route::get('referral/{code}', ApplyReferralCodeController::class)
    ...
    ->where('code', '[A-Z0-9]{8}')
    ->middleware('throttle:referral-claim');
...
Route::post('order/{order:order_number}/verify', [VerifyOrderAccessController::class, 'store'])->name('order.verify.store')->middleware('throttle:sensitive-write');
...
Route::get('pickup-slots/{date}', PickupSlotsController::class)
    ...
    ->middleware('throttle:frequent-poll');
...
Route::post('coupon/apply', ApplyCouponController::class)->name('coupon.apply')->middleware('throttle:form-write');

// Gift card validation (AJAX)
Route::post('gift-card/apply', ApplyGiftCardController::class)->name('giftCard.apply')->middleware('throttle:form-write');
