...
test('renders the success view without finalizing checkout when no session_id is provided', function () {
    ...
    $checkout->expects('handleCheckoutComplete')->never();
...
test('finalizes the checkout via the service when session_id is provided and deposit is unpaid', function () {
    ...
    $checkout = Double::for(CateringDepositCheckoutService::class);
    $checkout->expects('handleCheckoutComplete')
        ->with('cs_test_abc')
        ->resolves(function (mixed ...$arguments) use ($inquiry): CateringInquiry {
            $inquiry->forceFill(['deposit_paid_at' => now()])->save();

            return CateringInquiry::query()->whereKey($inquiry->getKey())->firstOrFail();
        });
    app()->instance(CateringDepositCheckoutService::class, $checkout);
