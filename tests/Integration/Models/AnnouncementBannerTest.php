...
test('announcement settings can be stored', function () {
    ...
    expect(settings('announcement_enabled'))->toBe('1');
    expect(settings('announcement_text'))->toBe('We are closed for the holiday!');
    expect(settings('announcement_type'))->toBe('warning');
});
...
test('announcement text defaults to empty', function () {
    expect(settings('announcement_text', ''))->toBe('');
});
