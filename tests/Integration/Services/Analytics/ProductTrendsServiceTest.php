...
test('returns products with order counts for the given month', function () {
    ...
    $trend = trendProduct($result);

    expect($result)->toHaveCount(1)
        ->and($trend['name'])->toBe($product->name)
        ->and($trend['current'])->toBe(5);
});
...
test('includes change percentage comparing to previous month', function () {
    ...
    expect($productData['current'])->toBe(15)
        ->and($productData['previous'])->toBe(10)
        ->and($productData['change'])->toBe(50.0);
});
