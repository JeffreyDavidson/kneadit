...
@if($order->delivery_address)
    <div class="delivery-info">
        ...
        <p style="margin: 5px 0;"><strong>Delivery Date:</strong> {{ $order->requested_date->format('M j, Y') }}</p>
        <p style="margin: 5px 0;"><strong>Delivery Time:</strong> {{ $order->requested_time->format('g:i A') }}</p>
...
<div style="background-color: #fff3cd; border-radius: 6px; padding: 15px; margin: 20px 0; border-left: 4px solid #ffc107;">
    ...
    <p style="margin: 5px 0;"><strong>Estimated completion:</strong> {{ $order->requested_date->format('M j, Y') }} at {{ $order->requested_time->format('g:i A') }}</p>
</div>
