...
>
    ...
    >
        ...
        <div style="background: linear-gradient(135deg, {{ $primary }}22 0%, {{ $secondary }}22 100%); padding: 40px 32px; text-align: center; border-bottom: 1px solid rgba(255,255,255,0.06);">
            @if ($logoPath)
                ...
                />
            @endif
            <h2 style="color: #fef9ef; margin: 0; font-size: 28px; font-weight: 700; letter-spacing: -0.5px">
                {{ $page->welcome['bakery_name'] ?: 'Your Bakery' }}
            </h2>
            <p style="color: {{ $primary }}; margin: 6px 0 0; font-size: 14px; font-weight: 600; text-transform: uppercase; letter-spacing: 1px;">
                {{ $page->welcome['owner_name'] }}
            </p>
        ...
        <div style="padding: 28px 32px; display: grid; grid-template-columns: 1fr 1fr; gap: 24px">
            ...
            >
                <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 12px">
                    <x-heroicon-o-envelope class="h-4 w-4" style="color: {{ $primary }};" stroke-width="2" />
                    <span
                        style="
                            color: #fef9ef;
                            font-size: 13px;
            ...
            >
                <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 12px">
                    <x-heroicon-o-truck class="h-4 w-4" style="color: {{ $primary }};" stroke-width="2" />
                    <span
                        style="
                            color: #fef9ef;
                            font-size: 13px;
                ...
                <div style="display: flex; flex-direction: column; gap: 6px">
                    <span style="display: inline-flex; align-items: center; gap: 6px; font-size: 13px; color: {{ $page->delivery['delivery_enabled'] ? '#6ee7b7' : '#6b4c3b' }};">
                        ...
                        Delivery{{ $page->delivery['delivery_enabled'] && $page->delivery['delivery_fee'] ? ' — ' : '' }}
                        @if ($page->delivery['delivery_enabled'] && $page->delivery['delivery_fee']) @money((float) $page->delivery['delivery_fee'])@endif
                    </span>
            ...
            >
                <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 12px">
                    <x-heroicon-o-clock class="h-4 w-4" style="color: {{ $primary }};" stroke-width="2" />
                    <span
                        style="
                            color: #fef9ef;
                            font-size: 13px;
            ...
            >
                <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 12px">
                    <x-heroicon-o-credit-card class="h-4 w-4" style="color: {{ $primary }};" stroke-width="2" />
                    <span
                        style="
                            color: #fef9ef;
                            font-size: 13px;
            ...
            <div style="padding: 0 32px 28px">
                <div style="background: linear-gradient(135deg, {{ $primary }}15 0%, {{ $secondary }}10 100%); border: 1px solid {{ $primary }}33; border-radius: 12px; padding: 20px; display: flex; justify-content: space-between; align-items: center;">
                    <div>
                        ...
                        <div style="color: #fef9ef; font-size: 16px; font-weight: 600; margin-top: 4px">
                            {{ $page->product['name'] }}
                        </div>
                        @if ($page->product['description'] ?? '')
                            <div style="color: #d4a574; font-size: 12px; margin-top: 4px; max-width: 400px">
                                {{ Str::limit($page->product['description'], 80) }}
                            </div>
                        @endif
                    </div>
                    <div style="color: #fef9ef; font-size: 24px; font-weight: 700">
                        @money((float) $page->product['price'])
                    </div>
                </div>
            </div>
        @endif
    ...
    <div style="text-align: center; margin-top: 24px; padding: 0 16px">
        <p style="color: #d4a574; font-size: 14px; margin: 0">
            Everything look good? Hit <strong style="color: #fef9ef">Finish</strong> to launch your bakery. <br /><span
                style="font-size: 12px; color: #6b4c3b"
                >You can change all of this later from your admin dashboard.</span>
        </p>
    </div>
</div>
