<x-filament-panels::page>
    @if ($viewingMessage && $this->getViewingRecord())
        ...
        <div class="mb-4">
            <button wire:click="backToList" class="text-sm flex items-center gap-1 text-golden">
                <x-heroicon-o-arrow-left class="w-4 h-4" />
                Back to messages
            </button>
        </div>

        <h2 class="text-lg font-bold mb-4 text-golden">{{ $record->subject }}</h2>
        ...
        <div class="space-y-4">
            ...
            <div @class([
                ...
                <div class="prose prose-sm max-w-none text-butter">
                    {!! nl2br(e($record->body)) !!}
                </div>
            </div>
            ...
            <div class="rounded-xl border border-honey bg-espresso p-4">
                <h3 class="text-sm font-semibold mb-2 text-golden">Reply</h3>
                <form wire:submit="sendReply">
                    <textarea wire:model="replyBody" rows="4" placeholder="Type your reply..."
                              class="w-full rounded-lg border border-honey bg-warm-black p-3 text-sm text-butter"></textarea>
                    @error('replyBody')
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-xs mt-0.5 text-butter">{{ Str::limit($msg->body, 80) }}</p>
                            </div>
                        </div>
                        <span class="text-xs whitespace-nowrap text-butter">{{ $msg->created_at->diffForHumans() }}</span>
                    </div>
                </div>
            @empty
                <div class="text-center py-8 text-butter">
                    <x-heroicon-o-envelope class="mx-auto mb-2 w-12 h-12 text-honey" />
                    <p>No messages yet</p>
                </div>
            @endforelse
        </div>
