<x-filament-panels::page>
    ...
    @if ($activeTab === 'health')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_minmax(0,2fr)] gap-4 mb-6">
            <x-central.card>
                ...
                <p class="text-cinnamon text-[0.75rem] mt-2">Across {{ $total }} active {{ Illuminate\Support\Str::plural('bakery', $total) }}.</p>
            </x-central.card>
            ...
            <x-central.card>
                <div class="flex items-center justify-between mb-3">
                    ...
                    <div class="flex items-center gap-4 text-[0.75rem]">
                        <span class="inline-flex items-center gap-1.5 text-parchment">
                ...
                <div class="flex h-2 rounded-full overflow-hidden bg-espresso">
                    <div class="bg-emerald-500 h-full" style="width: {{ round(($stats['healthy'] / $total) * 100, 2) }}%;"></div>
                    <div class="bg-amber-500 h-full" style="width: {{ round(($stats['at_risk'] / $total) * 100, 2) }}%;"></div>
                    <div class="bg-red-500 h-full" style="width: {{ round(($stats['critical'] / $total) * 100, 2) }}%;"></div>
                </div>

                <div class="grid grid-cols-3 gap-3 mt-3">
                    <div>
                        <div class="text-[1.5rem] font-bold text-emerald-400 leading-none">{{ $stats['healthy'] }}</div>
                        <div class="text-cinnamon text-[0.7rem] mt-1 uppercase tracking-[0.08em]">Healthy</div>
                    </div>
                    <div>
                        <div class="text-[1.5rem] font-bold text-amber-400 leading-none">{{ $stats['at_risk'] }}</div>
                        <div class="text-cinnamon text-[0.7rem] mt-1 uppercase tracking-[0.08em]">At risk</div>
                    </div>
                    <div>
                        <div class="text-[1.5rem] font-bold text-red-400 leading-none">{{ $stats['critical'] }}</div>
                        <div class="text-cinnamon text-[0.7rem] mt-1 uppercase tracking-[0.08em]">Critical</div>
                    </div>
                </div>
            </x-central.card>
        </div>
        ...
        @if ($tenants->isEmpty())
            <x-central.card padding="py-16 px-8" class="text-center">
                <x-heroicon-o-check-circle class="w-10 h-10 inline-block text-emerald-500 mb-3" />
                <div class="text-white font-semibold">No bakeries to evaluate yet.</div>
            </x-central.card>
        @else
            <div class="flex items-center justify-between mb-3">
                        ...
                        <div class="flex items-start gap-4 mb-4">
                            <div class="relative flex-shrink-0 w-14 h-14">
                                <svg viewBox="0 0 48 48" class="w-14 h-14 -rotate-90">
                                    ...
                                    <circle cx="24" cy="24" r="20" fill="none" class="{{ $ring['stroke'] }}" stroke-width="4" stroke-linecap="round"
                            ...
                            <div class="flex-1 min-w-0">
                                <a href="{{ $this->getViewTenantUrl($tenant['id']) }}"
                                    class="text-white font-semibold text-[0.95rem] hover:text-honey truncate block no-underline">
                                    {{ $tenant['name'] }}
                                </a>
                                <div class="text-cinnamon text-[0.75rem] truncate mb-1.5">{{ $tenant['owner'] }}</div>
                                <x-central.badge color="honey-soft" class="border border-honey/25">{{ $tenant['plan'] }}</x-central.badge>
                            </div>
                        </div>
                        ...
                        <dl class="space-y-1.5">
                            @foreach ($factors as $factor)
                                ...
                                <div class="grid grid-cols-[80px_1fr_auto] items-center gap-2">
                                    <dt class="text-cinnamon text-[0.7rem] uppercase tracking-[0.06em]">{{ $factor['label'] }}</dt>
                                    <dd class="bg-espresso rounded-full h-1.5 overflow-hidden">
                                        <div class="h-full rounded-full {{ $barClass }}" style="width: {{ $pct }}%;"></div>
                                    </dd>
                                    <dd class="text-parchment text-[0.7rem] font-mono tabular-nums w-[44px] text-right">{{ $factor['value'] }}/{{ $factor['max'] }}</dd>
                                </div>
                            @endforeach
                        </dl>
                    </x-central.card>
    ...
    @if ($activeTab === 'churn')
        ...
        @else
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                <x-central.card class="bg-red-500/5 border-red-500/20">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <div class="text-cinnamon text-[0.7rem] uppercase tracking-[0.08em] mt-1">Critical</div>
                        </div>
                    </div>
                </x-central.card>
                <x-central.card class="bg-amber-500/5 border-amber-500/20">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-xl bg-amber-500/15 border border-amber-500/25 flex items-center justify-center">
                            <x-heroicon-o-clock class="w-5 h-5 text-amber-400" />
                        </div>
                        <div>
                            <div class="text-[1.75rem] leading-none font-bold text-amber-400">{{ $warningCount }}</div>
                            <div class="text-cinnamon text-[0.7rem] uppercase tracking-[0.08em] mt-1">Warnings</div>
                        </div>
                    </div>
                </x-central.card>
                <x-central.card>
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-xl bg-honey/15 border border-honey/25 flex items-center justify-center">
                            <x-heroicon-o-inbox class="w-5 h-5 text-honey" />
                        </div>
                        <div>
                            <div class="text-[1.75rem] leading-none font-bold text-white">{{ $alerts->count() }}</div>
                            <div class="text-cinnamon text-[0.7rem] uppercase tracking-[0.08em] mt-1">Total Alerts</div>
                        </div>
                    </div>
                </x-central.card>
            </div>
                    ...
                    <x-central.card class="{{ $tone['tint'] }} {{ $tone['border'] }} hover:border-honey/40 transition-colors">
                        <div class="flex items-start gap-4 flex-wrap">
                            ...
                            <div class="flex items-start gap-3 flex-1 min-w-[280px]">
                                ...
                                <div class="min-w-0 flex-1">
                                    <div class="flex items-center gap-2 flex-wrap mb-1">
                                        <a href="{{ $this->getViewTenantUrl($alert['tenant_id']) }}"
                                            class="text-white font-semibold text-[0.95rem] hover:text-honey no-underline truncate">
                                            {{ $alert['name'] }}
                                        </a>
                                    ...
                                    <div class="text-cinnamon text-[0.7rem] mt-1.5">Signed up {{ $alert['days_since_signup'] }} days ago</div>
                                </div>
                            </div>
                            ...
                            <div class="flex items-center gap-2 flex-shrink-0">
                                ...
                                @else
                                    <button type="button" wire:click="extendTrial('{{ $alert['tenant_id'] }}')" wire:loading.attr="disabled"
                                        class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[0.75rem] font-semibold bg-espresso text-parchment border border-honey/20 hover:border-honey/50 transition-colors cursor-pointer">
                                ...
                                @else
                                    <button type="button" wire:click="sendNudge('{{ $alert['tenant_id'] }}')" wire:loading.attr="disabled"
                                        class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[0.75rem] font-semibold bg-espresso text-parchment border border-honey/20 hover:border-honey/50 transition-colors cursor-pointer">
                                ...
                                <a href="{{ $this->getViewTenantUrl($alert['tenant_id']) }}"
                                    class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[0.75rem] font-semibold bg-honey text-warm-black hover:bg-golden transition-colors no-underline">
                                    View Tenant
