...
>
    @if ($this->isSize('md'))
        <x-tenant-admin.dashboard.stat-row
            label="Active orders"
            :value="array_sum(array_column($stages, 'count'))"
            class="mb-2"
        />
    @endif
    ...
    @foreach ($stages as $stage)
        ...
        @endphp
        <x-tenant-admin.dashboard.list-row :dot-color="$stage['dotColor'] ?? null">
            <span style="color: var(--pw-card-text-muted); margin-right: 6px">{{ $stage['count'] }}</span>
            @if ($href)
                <a
                    href="{{ $href }}"
                    style="color: var(--pw-card-text); text-decoration: none"
                >{{ $stage['label'] }}</a>
            @else
