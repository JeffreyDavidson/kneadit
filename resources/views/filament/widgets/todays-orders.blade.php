        ...
        @if ($isLarge)
            ...
            @foreach ($rows as $row)
                <x-admin.dashboard.list-row :dot-color="$row['dot_color']" :value="$row['total']">
                    ...
                    @if ($hasViewRoute)
                        <a
                            href="{{ route('filament.admin.resources.orders.view', $row['id']) }}"
                            style="color: var(--pw-card-accent); text-decoration: none; margin-left: 8px"
                        >{{ $row['order_number'] }}</a>
                    @else
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(110px, 1fr)); gap: 6px">
                @foreach ($rows as $row)
                    ...
                    >
                        ...
                        <div style="font-size: 0.85rem; font-weight: 700; color: var(--pw-card-text)">
                            {{ $row['total'] }}
                        </div>
                        <div
                            style="
