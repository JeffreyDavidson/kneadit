<x-filament-widgets::widget>
    <div class="dashboard-action-panel">
        <div>
            ...
            <p class="dashboard-action-panel__copy">Create an order, triage the queue, or answer customer messages from one steady place.</p>
        </div>
            ...
            @if (\Illuminate\Support\Facades\Route::has('filament.admin.pages.quick-order'))
                <a href="{{ route('filament.admin.pages.quick-order') }}"
                   class="dashboard-action-button dashboard-action-button--primary">
                    <x-heroicon-s-plus-circle class="w-4 h-4" stroke-width="2.5" />
                    New Order
                </a>
            @endif
