...
<x-tenant-admin.dashboard.preview-card heading="Reorder Reminders" icon="heroicon-o-arrow-path">
    <x-tenant-admin.dashboard.stat-row label="Customers needing a nudge" :value="$count" class="mb-1" />
    <div style="font-size: 0.6rem; color: var(--pw-card-text-muted); margin-bottom: 8px">
        Ordered 2+ times, inactive 30+ days
    </div>

    @if (count($customers) > 0)
        @foreach ($customers as $customer)
            <x-tenant-admin.dashboard.list-row :value="$customer['last_order']">
