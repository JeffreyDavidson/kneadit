    ...
    <div class="preview-widget-body">
        @switch ($widget['key'])
            ...
            @case ('revenue_chart')
                ...
                @endphp
                <div style="font-size: 0.6rem; color: var(--accent); font-weight: 600; margin-bottom: 4px">
                    {{ $label }}
                </div>
            ...
            @case ('upcoming_orders')
                ...
                @if (($widget['size'] ?? 'sm') === 'lg')
                    ...
                    >
                        <span>Total</span
                        ><span style="color: var(--accent); font-weight: 600">${{ array_sum($amounts) }}</span>
                    </div>
                @endif
                @break
            @case ('top_products')
            ...
            @case ('customer_insights')
                @if (($widget['size'] ?? 'sm') === 'md')
                    <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 8px">
                        <div class="pw-stat" style="flex-direction: column; align-items: flex-start">
                            <span class="pw-stat-label">New this week</span><span class="pw-stat-value">4</span>
                        </div>
                        <div class="pw-stat" style="flex-direction: column; align-items: flex-start">
                            <span class="pw-stat-label">Repeat rate</span><span class="pw-stat-value">62%</span>
                        </div>
                        <div class="pw-stat" style="flex-direction: column; align-items: flex-start">
                            <span class="pw-stat-label">Avg LTV</span><span class="pw-stat-value">$184</span>
                        </div>
                        <div class="pw-stat" style="flex-direction: column; align-items: flex-start">
                            <span class="pw-stat-label">Total active</span><span class="pw-stat-value">128</span>
                        </div>
                    </div>
                @else
                    <div class="pw-stat">
                    ...
                    <div class="pw-stat" style="margin-top: 8px">
                        <span class="pw-stat-label">Repeat rate</span><span class="pw-stat-value">62%</span>
                    </div>
                @endif
                @break
                        ...
                        <div class="pw-line" style="height: 32px">
                            @foreach ([60, 75, 80, 95] as $i => $h)
                                ...
                                >
                                    <div
                                        class="pw-line-bar"
                                        style="height: {{ $h }}%; background: var(--accent); width: 100%;"
                                    ></div>
            ...
            @case ('order_funnel')
                ...
                @if (($widget['size'] ?? 'sm') === 'md')
                    <div class="pw-stat" style="margin-bottom: 6px">
                        <span class="pw-stat-label">Total Active</span><span class="pw-stat-value">26</span>
                    </div>
                @endif
            ...
            @case ('baking_sheet')
                ...
                @if (($widget['size'] ?? 'md') === 'lg')
                    <div class="pw-stat" style="margin-bottom: 6px">
                        <span class="pw-stat-label">Progress</span
                        ><span class="pw-stat-value">{{ $done }}/{{ count($items) }}</span>
                    </div>
                @endif
            ...
            @case ('inbox')
                ...
                @if (($widget['size'] ?? 'sm') === 'md')
                    @foreach (['Sarah M.' => 'Can I add to my order?', 'Mike R.' => 'Question about delivery time', 'Lisa K.' => 'Cake feedback - thanks!'] as $name => $msg)
                        <div class="pw-row">
                            <span style="color: var(--brand-100); font-weight: 600">{{ $name }}</span
                            ><span
                                style="color: var(--brand-400); font-style: italic"
                                >{{ \Illuminate\Support\Str::limit($msg, 24) }}</span>
                        </div>
                    @endforeach
                @else
                    <div style="font-size: 0.65rem; color: var(--brand-400); margin-top: 4px">
                        Latest: "Can I add to my order?"
                    </div>
                @endif
                @break
            @case ('margin_alert')
                ...
                @if (($widget['size'] ?? 'sm') === 'md')
                    <div class="pw-stat" style="margin-bottom: 6px">
                        <span class="pw-stat-label">At-risk products</span><span class="pw-stat-value">4</span>
                    </div>
                @endif
            ...
            @case ('coupon_usage')
                ...
                @foreach ($coupons as $code => $uses)
                    <div class="pw-row" @if ($loop->first) style="margin-top: 6px" @endif>
                        <span>{{ $code }}</span><span>{{ $uses }} uses</span>
                    </div>
                @endforeach
                @break
            @case ('gift_card_balance')
                <div class="pw-stat">
                    <span class="pw-stat-label">Outstanding</span><span class="pw-stat-value">$340</span>
                </div>
            ...
            @case ('review_summary')
                ...
                @if (($widget['size'] ?? 'sm') === 'md')
                    <div style="margin-top: 8px; padding: 6px 8px; background: var(--brand-900); border-radius: 6px">
                        ...
                        <div style="font-size: 0.65rem; color: var(--brand-100); font-style: italic; line-height: 1.4">
                            "Best chocolate cake I've ever had! Will be back."
                        </div>
                        <div style="font-size: 0.55rem; color: var(--brand-400); margin-top: 2px">
                            — Sarah M., 2 days ago
                        </div>
                    </div>
                @endif
            ...
            @case ('reorder_reminders')
                ...
                <div style="font-size: 0.6rem; color: var(--brand-400); margin-top: 4px">
            ...
            @case ('low_stock')
                ...
                @if (($widget['size'] ?? 'sm') === 'md')
                    <div class="pw-stat" style="margin-bottom: 6px">
                        <span class="pw-stat-label">Items at risk</span><span class="pw-stat-value">5</span>
                    </div>
                @endif
                @foreach ($items as $item => $remaining)
                    <div class="pw-row">
                        <span>{{ $item }}</span><span style="color: #d4574a">{{ $remaining }}</span>
                    </div>
                @endforeach
                @break
