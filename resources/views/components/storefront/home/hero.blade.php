@php
    $heroSettings = resolve(\App\Services\Settings\TenantSettings::class);
    $storeName ??= $heroSettings->store->name;
    $tagline ??= $heroSettings->branding->businessTagline;
    $aboutUs ??= $heroSettings->branding->aboutUsText;
    $heroStyle ??= $heroSettings->branding->heroStyle;
    $heroImageUrl ??= $heroSettings->branding->heroImageUrl();
    $heroTagline ??= $heroSettings->branding->heroTagline;
    $primaryCtaText ??= $heroSettings->branding->heroPrimaryCtaText;
    $secondaryCtaText ??= $heroSettings->branding->heroSecondaryCtaText;
    $customerCount ??= 0;
    $avgRating ??= null;
    $topReview ??= null;
@endphp
@if ($heroSettings->branding->storefrontTheme === 'biscotto')
    <section class="biscotto-hero" id="home">
        <img src="{{ $heroImageUrl }}" alt="Freshly baked sourdough bread" class="biscotto-hero-image" />
        <div class="biscotto-hero-overlay"></div>
        <div class="biscotto-hero-content">
            <div class="biscotto-hero-copy">
                <h1>{{ $storeName }}</h1>
                <p class="biscotto-hero-tagline">✦ {{ $heroTagline ?: 'Where Sourdough Dreams Come True' }} ✦</p>
                <p class="biscotto-hero-description">
                    Handcrafted sourdough and baked goods from our cottage kitchen to your table. Every creation starts
                    with care, patience, and our beloved starter.
                </p>
                <a href="{{ route('storefront.menu') }}" class="biscotto-hero-button"> Explore Our Menu&nbsp; → </a>
            </div>
        </div>
    </section>
@else
                        ...
                        >
                            @if ($customerCount > 0)
                                <div class="flex items-baseline" style="gap: 0.5rem">
                                    ...
                                    <dd
                                        class="font-display"
                                        style="font-size: 1.375rem; font-weight: 500; line-height: 1"
                                    >
