...
class CateringPipelineWidget extends Widget
{
    ...
    public function getTotalPipelineValue(): float
    {
        ...
        return $this->cached('value', [1800, 3600], fn (): float => DatabaseValue::float(DatabaseValue::int(CateringInquiry::query()->inPipeline()->sum('quoted_amount')) / 100));
    }
    ...
    public function getLatestInquiry(): ?CateringInquiry
    {
        ...
        $id = $this->cached('latest_id', [900, 1800], fn (): ?int => DatabaseValue::nullableInt(CateringInquiry::query()->latest()->value('id')));
